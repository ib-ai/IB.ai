...
public final class NoteCommand extends Command {
    ...
    protected void execute(CommandContext context) {
        if(context.getArguments().length == 0) {
            context.reply(__(context, "error.note_empty"));
            return;
        }
        Member member = UInput.getMember(context.getGuild(), context.getArguments()[0]);
        if(member == null) {
            context.reply(__(context, "error.note_invalid"));
            return;
        }
        ...
        if(context.getArguments().length == 1) {
            ...
            embedBuilder.setDescription(__(context, "info.note"));
            noteData.values().stream()
                    ...
                    .forEach(it -> {
                        ...
                        embedBuilder.addField(__(context, "info.note_author", user),
                                data,
                                false);
                    });
            ...
            if(note.length() > MessageEmbed.VALUE_MAX_LENGTH) {
                context.reply(__(context, "error.note_long", String.valueOf(MessageEmbed.VALUE_MAX_LENGTH)));
                return;
            }
            if(noteData.size() >= 25) {
                context.reply(__(context, "error.note_full"));
                return;
            }
            String data = context.getMember().getUser().getId() + ":" + note;
            noteData.add(data);
            gravity.save(noteData);
            context.reply(__(context, "success.note"));
        }
    }
