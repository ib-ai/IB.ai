...
public final class HelperMessageDelete implements CommandAction {

    /**
     * Deletes the helper message.
    ...
    public void accept(CommandContext context) {
        ...
        if (helperMessageRolesData.contains(role.getId())) {
            helperMessageData.getKeys().stream()
                    .filter(key -> key.equalsIgnoreCase(channel.getId())).forEach(key -> {
                        TextChannel channelOld = UInput.getChannel(context.getGuild(), key);
                        if (channelOld != null) {
                            channelOld.deleteMessageById(helperMessageData.get(key).asString())
                                .queue();
                        }
                    });
            helperMessageData.unset(channel.getId());

            if (helperMessageData.getKeys().size() == 0) {
                helperMessageRolesData.remove(role.getId());
            }
