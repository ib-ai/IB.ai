...
public class CMDBan extends CommandAbstract {
    private String banMessageID = null; //Mutated by lambda expression.
    ...
    public void execute(GuildMessageReceivedEvent event,
        ...
        try{
            ...
            if(message.getMentionedUsers().isEmpty()){
                event.getChannel().sendMessage("Correct usage: `" + Redis.getInstance().getGuildPrefix(guild) + "Ban @User` or `"
                        + Redis.getInstance().getGuildPrefix(guild) + "Ban [UserID]`").queue();
                return;
            }
        }
        ...
        User userToBan;
        if(!(message.getMentionedUsers().isEmpty())){
            userToBan = message.getMentionedUsers().get(0);

        }else{

            userToBan = guild.getMemberById(args[0]).getUser();
        }

        guild.getController().ban(userToBan, 0).complete();
        ...
        if(!Redis.getInstance().getGuildModLogID(guild).equals("000")){
            ...
            String bannedUserName = userToBan.getName() + "#" + userToBan.getDiscriminator();
            String bannedUserID = userToBan.getId();
            ...
            guild.getTextChannelById(modLogChannelID).sendMessage(
                    "**Case: #" + caseNumber + " | Ban :hammer:**\n" +
                            "**Offender: **" + bannedUserName + " (ID: " + bannedUserID + ")\n" +
                    ...
                    .queue(owo -> Redis.getInstance().addUserBan(guild, bannedUserID, caseNumber, owo.getId()));
